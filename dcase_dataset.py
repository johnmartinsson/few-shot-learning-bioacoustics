...
class BioacousticDataset(torch.utils.data.Dataset):
        ...
        def __init__(self, root_dir, window_size, hop_size, sample_rate, n_classes, n_time, include_background=False, transform=None, cache=True, is_validation_data=False):
                """
                Args:
                ...
                if cache and os.path.exists(os.path.join(cache_dir, x_path)):
                    ...
                    #csv_paths = [csv_path] #glob.glob(os.path.join(root_dir, '*/*.csv')) #[:1]
        ...
        def __init__(self, root_dir, window_size, hop_size, sample_rate, n_classes, n_time, include_background=False, transform=None, cache=True):
                ...
                if cache and os.path.exists(os.path.join('cache', x_path)):
                        ...
                        for wav_path, csv_path in tqdm.tqdm(list(zip(wav_paths, csv_paths))):
                                ...
                                sig_segs, sig_seg_targets, bg_segs, bg_seg_targets = get_segments_and_labels(
                                        wave, sample_rate, annotation_df, hop_size, window_size, n_classes, n_time, get_label_train_fn
                                )
